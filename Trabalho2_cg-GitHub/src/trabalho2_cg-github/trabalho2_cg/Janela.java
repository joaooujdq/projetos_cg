...
public class Janela extends JFrame {
	...
	private void createMenuBar() {
		...
		analiticoItem.addActionListener((event) -> new JanelaEspecifica(analiticoItem.getText(), cor));

		linhasMenu.add(DDAItem);
		DDAItem.addActionListener((event) -> new JanelaEspecifica(DDAItem.getText(), cor));

		linhasMenu.add(poliBresItem);
		poliBresItem.addActionListener((event) -> new JanelaEspecifica(poliBresItem.getText(), cor));

		poliMenu.add(varreduraItem);
		varreduraItem.addActionListener((event) -> new JanelaEspecifica(varreduraItem.getText(), cor));

		poliMenu.add(boundaryItem);
		boundaryItem.addActionListener((event) -> new JanelaEspecifica(boundaryItem.getText(), cor));

		poliMenu.add(geoItem);
		geoItem.addActionListener((event) -> new JanelaEspecifica(geoItem.getText(), cor));

		circMenu.add(parametricaItem);
		parametricaItem.addActionListener((event) -> new JanelaEspecifica(parametricaItem.getText(), cor));

		circMenu.add(incSimItem);
		incSimItem.addActionListener((event) -> new JanelaEspecifica(incSimItem.getText(), cor));

		circMenu.add(circBresItem);
		circBresItem.addActionListener((event) -> new JanelaEspecifica(circBresItem.getText(), cor));
